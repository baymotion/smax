...
def compile_state_machine(
    filename, generated_source_filename="%(dirname)s/.generated.%(basename)s"
):
    ...
    if generated_source_filename:
        ...
        module_name = basename.rstrip(".py") + "%04X" % (
            hash(state_machine_source) & 0xFFFF,
        )
    ...
    class TestFramework(state_machine_class):
        ...
        def expected(self, event_list):
            ...
            try:
                ev = next(i)
                log._debug(
                    "Unexpected %s when it should have been empty." % (ev,),
                )
                assert False
            except queue.Empty:
                pass
