...
def main():
    parser = argparse.ArgumentParser(description="Translate a smax script to python.")
    parser.add_argument("--output", help="Write the output to the given filename")
    parser.add_argument("--verbose", action="store_true", help="Enable trace-level messages")
    parser.add_argument("--yaml", help="Yaml filename to write")
    parser.add_argument("input", help="input script; use '-' for standard input.")
    ...
    output_filename = "/dev/stdout" if args.output == None else args.output
