# Copyright (C) 2016, Patrick O'Grady.  All rights reserved.
#
...
#   Pass a parameter 'delimiter="XY"' to use XY instead of %% as delimiters
#   or 'start_delimiter="%{", end_delimiter="%}"' to use different
#   delimiters to start and end.
#
def load_source(filename, delimiter="%%", start_delimiter=None, end_delimiter=None):
    if start_delimiter == None:
        start_delimiter = delimiter
    if end_delimiter == None:
...
parser state_machine:
    ...
    rule _expr<<l>>:
        ...
        |   OPEN_PAREN _expr<<[]>> CLOSE_PAREN {{ l.append("("); l.extend(_expr); l.append(")") }}
        )
        {{ return l }}
...
class Scanner(state_machineScanner):
    ...
    def token(self, restrict, context=None):
        ...
        if ("INDENT" in restrict) or ("DEDENT" in restrict):
            ...
            log.trace("Checking in/dedent, m=%s (length=%s)." % (m, "n/a" if m is None else len(m.group(1))))
            if m:
                indent = len(m.group(1))
                log.trace("indent=%u." % indent)
                if indent > self._indent[-1]:
                    self._indent.append(indent)
                    token = yapps.runtime.Token("INDENT", indent, self.get_pos())
                    log.trace("token=%s." % (token,))
                    return token
                if (indent < self._indent[-1]):
                    self._indent.pop()
                    token = yapps.runtime.Token("DEDENT", indent, self.get_pos())
                    log.trace("token=%s." % (token,))
                    return token
        if ("INDENTED_CODE" in restrict):
            ...
            if m:
                indent = len(m.group(1))
                code = m.group(2)
                log.trace("indent=%u (was %u), code=%s." % (indent, self._indent[-1], code))
                if indent >= self._indent[-1]:
                    code_spaces = " " * (indent - self._indent[-1])
                    line = code_spaces + code
                    token = yapps.runtime.Token("INDENTED_CODE", line, self.get_pos())
                    log.trace("token=%s." % (token,))
        ...
        for superclass in superclasses:
            ...
            if existing_args:
                if len(args) != len(existing_args):
                    raise Exception("Incompatible parameter list with %s superclass %s." % \
                        self.name, name)
                continue
...
class State(object):
    def __init__(self, machine, parent, name, start):
        ...
        self._events = { }
        self.events = [ ]
#       self._full_name = copy.copy(parent._full_name) if parent else []
...
#       self.dot_name = ".".join(self._full_name)
        ...
        for sl in self.inner_states:
            ...
            for s in sl:
                if s.start and start_found:
                    raise Exception("Multiple start states found in %s." % self.name)
                if s.start:
                    start_found = True
            if not start_found:
    ...
    def set_exit(self, exit):
        self.exit = exit
    def add_transition(self, event_name, event_args, superclasses, condition, state_target, code_clause):
        ev = self._machine.event(event_name, event_args, superclasses)
        t = Transition(self, ev, superclasses, condition, state_target, code_clause)
...
class Machine(State):
    ...
    def event(self, event, event_args, superclasses):
        event_name = event
        log.trace("event_name=%s, event_args=%s, superclasses=%s." % (event_name, event_args, superclasses))
        try:
...
class Specification:
    ...
    def import_(self, l):
        log.trace("new import, l=%s." % (l,))
        self._output.append({"import": l})
